#include "Placer.h"
#include<algorithm>
#include<memory>
#include<cmath>
#include<queue>
#include<unordered_map>
using namespace std;

Cell Placer::makeCell(const Device* d) {
	Cell c;
	c.symbol = d->symbol;
	c.device = d;
	c.isDummy = false;
	return c;
}

double Placer::distanceFromCenter(int i, int j, double centerRow, double centerCol) {
	double dr = i - centerRow;
	double dc = j - centerCol;
	return sqrt(dr * dr + dc * dc); // eucludean distance
}
int Placer::planOddPairing(vector<const Device*>& devices, vector<bool>& pairPrev)
{
    int i = 0;
    bool unpairedPrev = false;
    int pendingIndex = -1;
    for (const Device* d : devices)
    {
        if (d->count % 2 == 1)
        {
            if (pendingIndex != -1)
            {
                pairPrev[i] = true;
                pendingIndex = -1;
            }
            else pendingIndex = i;
        }
        i++;
    }
    return pendingIndex;
}

Grid Placer::buildPlacement(const TestCase& tc) {
    priority_queue<PairSlot, vector<PairSlot>, ClosestFirst> pairQueue;
    int total_real = 0, numOdd = 0;
    for (const Device& d : tc.devices) {
        total_real += d.count;
        if (d.count % 2 == 1) numOdd++;
    }
    // if there is an odd number of leftovers , need to add a dummy to make it even
    int total_counts = total_real + (numOdd % 2); 

    int cols = ceil(total_counts / (double)tc.rows);

    pair<double, double> centerCell = { (tc.rows - 1) / 2.0, (cols - 1) / 2.0 };
    vector<vector<bool>> visited(tc.rows, vector<bool>(cols, false));
    Grid grid;
    grid.rows = tc.rows;
    grid.cols = cols;
    grid.cells.assign(tc.rows, vector<Cell>(cols));

    for (int j = 0; j < cols; j++) {
        for (int i = 0; i < tc.rows; i++) {
            grid.cells[i][j] = { '\0', false, nullptr, nullptr };
            int mirror_i = tc.rows - 1 - i;
            int mirror_j = cols - 1 - j;
            if (!visited[i][j] && !(i == mirror_i && j == mirror_j)) {
                double d = distanceFromCenter(i, j, centerCell.first, centerCell.second);
                pairQueue.push({ {i,j}, {mirror_i,mirror_j}, d });
                visited[mirror_i][mirror_j] = true;
                visited[i][j] = true;
            }
        }
    }

    vector<const Device*> devices;
    for (const auto& dev : tc.devices) devices.push_back(&dev);
    // sort devices by symbol
    sort(devices.begin(), devices.end(), [](const Device* a, const Device* b) { return a->symbol < b->symbol; });

    queue<const Device*> leftovers; // odd-count devices, alphabetical order
    vector<bool> pairPrev(devices.size(), false);
    int dummyIndex = planOddPairing(devices, pairPrev);

    int i = 0;
    for (const Device* d : devices) {
        int remaining = d->count;
        if (remaining % 2 == 1)
        {
            // either pair with previous or queue as leftover to be paired later
            if (pairPrev[i])
            {
                const Device* d1 = leftovers.front(); leftovers.pop();
                PairSlot p = pairQueue.top(); pairQueue.pop();
                grid.cells[p.a.row][p.a.col] = makeCell(d1);
                grid.cells[p.b.row][p.b.col] = makeCell(d);
            }
            else {
                leftovers.push(d);

            }
            remaining--;
        }
        while (remaining >= 2) {
            PairSlot p = pairQueue.top(); pairQueue.pop();
            grid.cells[p.a.row][p.a.col] = makeCell(d);
            grid.cells[p.b.row][p.b.col] = makeCell(d);
            remaining -= 2;
        }
        if (i == dummyIndex)
        {
            const Device* d = leftovers.front();
            leftovers.pop();
            PairSlot p = pairQueue.top(); pairQueue.pop();
            grid.cells[p.a.row][p.a.col] = makeCell(d);
            grid.cells[p.b.row][p.b.col].dummyRef = &tc.dummy;
            grid.cells[p.b.row][p.b.col].symbol = tc.dummy.symbol;
            grid.cells[p.b.row][p.b.col].isDummy = true;
        }

        i++;
    }

    for (int i = 0; i < grid.rows; ++i)
        for (int j = 0; j < grid.cols; ++j)
            if (grid.cells[i][j].symbol == '\0') {
                grid.cells[i][j].symbol = tc.dummy.symbol;
                grid.cells[i][j].dummyRef = &tc.dummy;
                grid.cells[i][j].isDummy = true;
            }

    return grid;
}